...
static size_t getIndex(MyVertex& v_, osg::ref_ptr<osg::Vec3Array> vertices)
{
    for (size_t i = 0; i < vertices->size(); i++) {
...
void OSGBMeshImpleMesh::write(const boost::filesystem::path& path_)
{
    if (path_.extension() == ".osgb" || path_.extension() == ".osgt") {
        ...
        for (size_t i = 0; i < m_nativeMesh.face.size(); i++) {
            ...
            indices->push_back(getIndex(*m_nativeMesh.face[i].V(0), vertices));
            indices->push_back(getIndex(*m_nativeMesh.face[i].V(1), vertices));
            indices->push_back(getIndex(*m_nativeMesh.face[i].V(2), vertices));
        }
